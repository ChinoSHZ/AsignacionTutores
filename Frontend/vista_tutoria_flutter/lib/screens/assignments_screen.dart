...
class _AssignmentsScreenState extends State<AssignmentsScreen> {
  ...
  void initState() {
    super.initState();
    _loadSemestres(); // cargar semestres al iniciar
  }

  // ── NUEVO: carga semestres desde el backend ────────────────
  ...
  List<Student> get _filtered {
    return widget.students.where((s) {
      ...
      // ── NUEVO: filtro por periodo de ingreso = semestre ────
      final matchSemestre =
          _filterSemestre == 'Todos' || s.entryPeriod == _filterSemestre;
      // ──────────────────────────────────────────────────────
  ...
  void _showReassignDialog(Student student) {
    final available =
        widget.tutors.where((t) => t.id != student.tutorId && t.count < 35).toList();
    showDialog(
      context: context,
            ...
            if (student == null) {
              ...
              if (index != -1) {
                final updatedStudent = Student(
                  ...
                  wasReassigned:
                      student.tutorId != tutorId || student.wasReassigned,
                  isActive: isActive,
                );
                ...
                if (student.tutorId != tutorId) {
                  widget.tutors
                      .firstWhere((t) => t.id == student.tutorId)
                      .students
                      .remove(student);
                  widget.tutors
                      .firstWhere((t) => t.id == tutorId)
                      .students
                      .add(updatedStudent);
                }
              }
  ...
  Widget build(BuildContext context) {
    ...
    return ScreenWrapper(
      ...
      subtitle:
          'Gestión manual de asignaciones. ${filtered.length} registros en total.',
      scrollable: false,
      child: Column(children: [
        ...
        Container(
          ...
          child: Column(children: [
            ...
            // ── Fila de filtros con semestre al final ─────────────────
            Row(children: [
              const Text('Filtros:',
                  style:
        ...
        Container(
          ...
          child: const Row(children: [
            ...
            SizedBox(
                width: 140,
                child: _TableHeader('Acciones', alignRight: true)),
          ]),
        ),
          ...
          child: paginatedStudents.isEmpty
              ...
              : ListView.separated(
                  ...
                  itemBuilder: (_, i) {
                    ...
                    final tutor = widget.tutors.firstWhere(
                        (t) => t.id == s.tutorId,
                    ...
                    return Container(
                      ...
                      child: Row(children: [
                        ...
                        Expanded(
                            ...
                            child: InkWell(
                              onTap:
                                  isLocked ? null : () => _showReassignDialog(s),
                              borderRadius: BorderRadius.circular(6),
                              child: Padding(
                                padding:
                                    const EdgeInsets.symmetric(vertical: 2.0),
                                  ...
                                  Container(
                                    ...
                                    child: Row(mainAxisSize: MainAxisSize.min,
                                        children: [
                                          ...
                                          Text(
                                              AppTheme.mobilityLabel(s.mobility),
                                              style: TextStyle(
                                                  color: mobilityColor,
                                                  fontSize: 10,
                                                  fontWeight: FontWeight.w600)),
        ...
        // Notas y paginación — sin cambios
        Container(
          padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 10),
          decoration: BoxDecoration(
            color: AppTheme.surfaceLight,
      ...
      content: SizedBox(
        ...
        child: DropdownButtonFormField<T>(
          ...
          items: widget.options
              .map((o) => DropdownMenuItem(
                  value: o,
                  child: Text(widget.labelBuilder(o),
                      style:
            ...
            children: [
                  ...
                  child: Row(children: [
                    ...
                    Text(currentTutor.name,
                        style: const TextStyle(
                            ...
                            fontSize: 13)),
                    const Spacer(),
                    Text('${currentTutor.count} alumnos',
                        style: TextStyle(
                            color: AppTheme.statusColor(currentTutor.status),
                            fontSize: 12))
                  ])),
              const SizedBox(height: 20),
              ...
              if (availableTutors.isEmpty)
                const Padding(
                    ...
                    child: Text(
                        'No hay tutores con capacidad disponible.',
                        style: TextStyle(color: AppTheme.textSecondary)))
              else
...
class _StudentDialogState extends State<_StudentDialog> {
  ...
  void initState() {
    ...
    _periodoCtrl =
        TextEditingController(text: s?.entryPeriod ?? '');

    final nameParts = s?.name.split(' ') ?? [];
    _nombreCtrl = TextEditingController(
        text: nameParts.isNotEmpty ? nameParts[0] : '');
    _apPaternoCtrl = TextEditingController(
        text: nameParts.length > 1 ? nameParts[1] : '');
    _apMaternoCtrl = TextEditingController(
        text:
            nameParts.length > 2 ? nameParts.sublist(2).join(' ') : '');

    final careerExists = widget.careersCatalog
        .any((c) => c.abbreviation == s?.career);
    _selectedCareer = (careerExists && s != null)
        ? s.career
        : (widget.careersCatalog.isNotEmpty
            ? widget.careersCatalog.first.abbreviation
            : '');

    _selectedMobility = s?.mobility ?? MobilityFlag.newStudent;
    _selectedTutorId = s?.tutorId ??
        (widget.tutors.isNotEmpty ? widget.tutors.first.id : '');
      ...
      child: Container(
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text(isEdit ? 'Editar Alumno' : 'Alta de Alumno',
                  style: const TextStyle(
                      color: AppTheme.textPrimary,
                      fontSize: 18,
                      fontWeight: FontWeight.w700)),
              ...
              if (!isEdit)
                _buildField('ID (Sistema)', _idCtrl, readOnly: true),
              ...
              Row(children: [
                Expanded(
                    child: _buildField(
                        'Número de Cuenta', _cuentaCtrl,
                        isNumber: true)),
                const SizedBox(width: 12),
                Expanded(
                    child: _buildField(
                        'Semestre (Ej: 2024A)', _periodoCtrl)),
              ]),
              const SizedBox(height: 16),

              DropdownButtonFormField<String>(
                value:
                    _selectedCareer.isEmpty ? null : _selectedCareer,
              ...
              Row(children: [
                Expanded(
                    child: DropdownButtonFormField<String>(
                  value: _selectedTutorId.isEmpty
                      ? null
                      : _selectedTutorId,
                ...
                Expanded(
                  ...
                  items: MobilityFlag.values
                      .map((m) => DropdownMenuItem(
                          value: m,
                          child: Text(AppTheme.mobilityLabel(m),
                              style: const TextStyle(
                                  color: AppTheme.textPrimary))))
                      .toList(),
                  onChanged: (v) =>
                      setState(() => _selectedMobility = v!),
                )),
              ]),
              const SizedBox(height: 16),
              ...
              SwitchListTile(
                ...
                subtitle: Text(
                    _isActive ? 'Activo (Cursando)' : 'Baja (Inactivo)',
                    style: TextStyle(
                        color:
                            _isActive ? AppTheme.green : AppTheme.red,
                        fontSize: 11)),
                value: _isActive,
